	...
	if (isset($_POST['submit_guess'])) {
		...
		if (correctNumber($newGuess)) {
			if (!$_SESSION['done']) {
				...
				$numberOfTries = count($_SESSION['tries']);
	
				$sql = 'INSERT INTO games(user_id, guesses) VALUES ('.$_SESSION['userId'].', '.$numberOfTries.')';
	...
	<body>
		<div class="container center" style="max-width: 800px;">
			<div class="card bg-light shadow px-5 pt-4 pb-3">
				<table class="w-100">
					<tr class="row mb-4">
						<td class="col-8 text-center" colspan="8"><h1>Guess the number</h1></td>
						<td class="col-2 text-center" colspan="2">
							<form name="form1" method="GET" action="<?php echo $_SERVER['PHP_SELF'];?>">
								<button type="submit" name="goto_stat" class="btn btn-dark" value="goto_stat">Stats</button>
							</form>
						</td>
						<td class="col-2 text-center" colspan="2">
							<form name="form1" method="GET" action="<?php echo $_SERVER['PHP_SELF'];?>">
